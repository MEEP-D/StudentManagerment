using StudentManagerment.Commands;
using StudentManagerment.Components;
...
namespace StudentManagerment.ViewModels
{
	public class LoginViewModel : BaseViewModel, INotifyDataErrorInfo
	{
				...
				else
				{
					...
					if (LoginServices.Instance.IsUserAuthentic(Username, Password))
					{
						try
						{
							...
							using (StreamWriter sw = new StreamWriter(LoginServices.RememberedAccountFilePath))
							{
								sw.Write(RememberedAccount.UserName + '\t' + LoginServices.Encrypt(RememberedAccount.PassWord, "LangTrack"));
							}
						}
						catch { }
					}
		...
		public void InitRememberedAccount()
		{
			...
			string filePath = LoginServices.RememberedAccountFilePath;
			if (File.Exists(filePath))
			{
				...
				using (StreamReader sr = new StreamReader(filePath))
				{
					...
					RememberedAccount = new Account(account[0], LoginServices.Decrypt(account[1], "LangTrack"));
				}
			}
			else
			{
